...
public class TicTacToeGame {
	...
	/**
	 * @return Optional<Player> winner. if does not exist then GAME ended without winner.
	 */
	...
	public boolean isGameCompleted() {
		if(history.size() == (this.gameField.length * this.gameField[0].length)) {return true;}
	...
	/**
	 * Was considering to traverse over positions in history
	 * Method ignore any index out of bounds exception and check do we have three in row in any direction.
	 * @param rowNum
	 * @param columnNum
	 * @return 
	 */
	private boolean checkEqualsAroundPosition(int rowNum , int columnNum) {
		...
		if(rowNum<0 || rowNum>this.gameField.length-1 || columnNum < 0 || columnNum > this.gameField[0].length-1 ) return false;
		if(this.gameField[rowNum][columnNum] == DEFAULT_VALUE) return false;
		
		//horizontal
		try {
			if(gameField[rowNum][columnNum] == gameField[rowNum][columnNum-1] 
				&& gameField[rowNum][columnNum] == gameField[rowNum][columnNum+1]) return true;
		}catch(IndexOutOfBoundsException e) {}
		//vertical
		try {
			if(gameField[rowNum][columnNum] == gameField[rowNum-1][columnNum] 
					&& gameField[rowNum][columnNum] == gameField[rowNum+1][columnNum]) return true;
		}catch(IndexOutOfBoundsException e) {}
		//lDiagonal
		try {
	...
	private void validatePlayedPosition(Position position)throws IllegalArgumentException {
		if(!isPosotionInScope(position)) throw new IllegalArgumentException("Position "+new Position(position.getRow()+1, position.getColumn())+" exceeds board size");
		if(DEFAULT_VALUE != this.gameField[position.getRow()][position.getColumn()]) throw new IllegalArgumentException("Position "+new Position(position.getRow()+1, position.getColumn()+1)+" is already used"); 
	}

	private boolean isPosotionInScope(Position position) {
		if(position.getRow()<0 
				|| position.getRow()> gameField.length-1
